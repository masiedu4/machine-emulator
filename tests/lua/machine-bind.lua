...
-- test send_cmio_response/log_send_cmio_response with different data sizes  and arguments
for data_name, data in pairs(load_cmio_test_data_set) do
    ...
    do_test("send_cmio_response happy path - " .. data_name, function(machine)
    ...
    for _, proofs in ipairs({ false, true }) do
        for _, large_data in ipairs({ false, true }) do
            ...
            do_test(
                string.format(
                    "log_send_cmio_response happy path - %s with proofs=%s, annotations=%s, large_data=%s",
                    data_name,
                    proofs,
                    annotations,
                    large_data
...
do_test("Dump of log produced by log_send_cmio_response should match", function(machine)
    machine:set_iflags_Y()
    ...
    print(actual_dump)
    print("--------------------------")
    assert(actual_dump == expected_dump, "Dump of send_cmio_response does not match expected:\n" .. expected_dump)
end)
