...
private:
    ...
    static std::pair<uint64_t, uint64_t> adjust_word_access(uint64_t paddr, uint64_t log2_size) {
    ...
    const auto &check_read_data(const access &a, const char *text) {
        ...
        if (a.get_read_hash() != computed_read_hash) {
            throw std::invalid_argument{"logged read data for " + std::string(text) +
                " does not match logged read hash in " + access_to_report()};
        }
        return a.get_read().value();
    }

    void check_written_data(const access &a, const char *text) {
        if (!a.get_written().has_value()) {
            return;
        }
        if (!a.get_written_hash().has_value()) {
            throw std::invalid_argument{"missing written hash for " + std::string(text) + " in " + access_to_report()};
        }
        // check if logged read data hashes to the logged read hash
        const auto computed_written_hash = get_hash(m_hasher, a.get_written().value());
        if (a.get_written_hash().value() != computed_written_hash) {
            throw std::invalid_argument{"logged written data for " + std::string(text) +
                " does not match logged written hash in " + access_to_report()};
        }
    }
    ...
    void check_write_word_access(uint64_t paddr, uint64_t val, const char *text) {
        ...
        [[maybe_unused]] const auto [access_paddr, access_log2_size] = adjust_word_access(paddr, log2_size);
        const int val_offset = paddr - access_paddr;
        replace_word_access_data(val, written_data, val_offset);
        const auto expected_written_hash = get_hash(m_hasher, written_data);
        const auto &written_hash = check_written_hash(a, expected_written_hash, text);
        check_written_data(a, text);
        update_root_hash(proof, written_hash);
        m_next_access++;
    }
    ...
    uint64_t check_read_word_access(uint64_t paddr, const char *text) {
        ...
        [[maybe_unused]] const auto [access_paddr, access_log2_size] = adjust_word_access(paddr, log2_size);
        const int val_offset = paddr - access_paddr;
        uint64_t val = get_word_access_data(read_data, val_offset);
        m_next_access++;
        return val;
    }
    ...
    uint64_t do_read_x(int i) {
        ...
        return check_read_word_access(machine_reg_address(reg), machine_reg_name(reg));
    }

    void do_write_x(int i, uint64_t val) {
        assert(i != 0);
        const auto reg = machine_reg_enum(machine_reg::uarch_x0, i);
        check_write_word_access(machine_reg_address(reg), val, machine_reg_name(reg));
    }
