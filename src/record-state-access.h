...
/// \file
/// \brief State access implementation that records to an access log all accesses performed by send_cmio_response
...
private:
    ...
    uint64_t log_read(uint64_t paligned, uint64_t val, const char *text) const {
        ...
        if (m_log->get_log_type().has_proofs()) {
            ...
            // 1) A full merkle tree update was called at the beginning of machine::log_send_cmio_response()
    ...
    void do_reset_iflags_Y(void) {
        ...
        log_read(iflags_addr, old_iflags, "iflags.Y");
    ...
    bool do_read_iflags_Y(void) const {
        log_read(shadow_state_get_csr_abs_addr(shadow_state_csr::iflags), m_m.get_state().read_iflags(), "iflags.Y");
    ...
    void do_replace_cmio_rx_buffer(const unsigned char *data, uint64_t length) {
        ...
        // find the PMA entry for the CMIO RX buffer
        ...
        access a{};
        a.set_type(access_type::write);
        a.set_address(PMA_CMIO_RX_BUFFER_START);
        a.set_log2_size(PMA_CMIO_RX_BUFFER_LOG2_SIZE);
        ...
        auto proof = m_m.get_proof(PMA_CMIO_RX_BUFFER_START, PMA_CMIO_RX_BUFFER_LOG2_SIZE);
        // log hash and data before write
        a.set_read_hash(proof.get_target_hash());
        if (m_log->get_log_type().has_large_data()) {
            access_data &data = a.get_read().emplace(PMA_CMIO_RX_BUFFER_LENGTH);
            memcpy(data.data(), pma.get_memory().get_host_memory(), PMA_CMIO_RX_BUFFER_LENGTH);
        }
        if (m_log->get_log_type().has_proofs()) {
            // We just store the sibling hashes in the access because this is the only missing piece of data needed to
            // reconstruct the proof
        ...
        // Write zero-padded data to the pma memory
        ...
        a.get_written_hash().emplace();
        hasher_type hasher{};
        get_merkle_tree_hash(hasher, pma.get_memory().get_host_memory(), PMA_CMIO_RX_BUFFER_LENGTH, sizeof(uint64_t),
            a.get_written_hash().value());
        if (m_log->get_log_type().has_large_data()) {
            access_data &data = a.get_written().emplace(PMA_CMIO_RX_BUFFER_LENGTH);
            memcpy(data.data(), pma.get_memory().get_host_memory(), PMA_CMIO_RX_BUFFER_LENGTH);
        }
        // NOLINTEND(bugprone-unchecked-optional-access)
