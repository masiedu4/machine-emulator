...
machine::machine(const machine_config &c, const machine_runtime_config &r) :
    ...
    if (m_c.rollup.has_value()) {
        ...
        register_pma_entry(make_callocd_memory_pma_entry("cmio rx buffer memory range"s, PMA_CMIO_RX_BUFFER_START,
            PMA_CMIO_RX_BUFFER_LENGTH)
                               .set_flags(m_rollup_rx_buffer_flags));
    }
...
access_log machine::log_send_cmio_response(uint16_t reason, const unsigned char *data, size_t length,
    ...
    if (log_type.has_proofs()) {
        ...
        verify_send_cmio_response_state_transition(reason, data, length, root_hash_before, *a.get_log(), root_hash_after,
            m_r, one_based);
    } else {
