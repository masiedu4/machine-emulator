...
public:
    ...
    explicit uarch_record_state_access(uarch_state &us, machine &m, access_log::type log_type) :
        ...
        const auto haddr = cast_ptr_to_host_addr(m_us.ram.get_memory_noexcept().get_host_memory());
        const auto paddr = m_us.ram.get_start();
        // initialize translation cache from paddr in uarch RAM to host address
        m_us_ram_ph_offset = haddr - paddr;
    }
        ...
        ~scoped_note() {
            if (m_log) {
                try {
                    ...
                    // unless the user is messing with unbalanced begin/end brackets, we cannot
                    // run out of memory in the end bracket.
                    // If we did, it was because the system is completely screwed anyway *and* the
                    // user is using unbalanced brackets.
                    // It's ok to quietly ignore, as the user's brackets were already unbalanced anyway...
                }
            }
        }
    };
...
private:
    ...
    static std::pair<uint64_t, uint64_t> adjust_access(uint64_t paddr, uint64_t log2_size) {
        ...
        const uint64_t log2_word_size = machine_merkle_tree::get_log2_word_size();
    ...
    void log_access_type(access &a, access_type type) const {
        a.set_type(type);
    }

    void log_access_range(access &a, uint64_t paddr, uint64_t log2_size) const {
        a.set_address(paddr);
        a.set_log2_size(log2_size);
    }

    void log_access_siblings_and_read_hash(access &a, uint64_t paddr, uint64_t log2_size) const {
    ...
    void log_written_hash(access &a, const hash_type &written_hash) const {
        a.get_written_hash().emplace(written_hash);
    }

    const access_data &log_read_data(access &a, uint64_t paddr, uint64_t log2_size) const {
    ...
    uint64_t log_read_reg_access(machine_reg reg) const {
        return log_read_word_access(machine_reg_address(reg), machine_reg_name(reg));
    }

    template <typename WRITE_F>
    void log_write_access(uint64_t paddr, uint64_t log2_size, WRITE_F write, const char *text) {
        access a;
        log_access_type(a, access_type::write);
        log_access_range(a, paddr, log2_size);
        const auto [access_paddr, access_log2_size] = adjust_access(paddr, log2_size);
        log_access_siblings_and_read_hash(a, access_paddr, access_log2_size);
        // Functor performs the write
        write();
        // The write functor updated the tree, so we don't do it again
    ...
    void log_write_reg_access(machine_reg reg, uint64_t val) {
        log_write_access(
            machine_reg_address(reg),
            cartesi::log2_size<uint64_t>::value,
            [this, reg, val]() {
    ...
    uint64_t do_read_word(uint64_t paddr) const {
        ...
        return log_read_word_access(paddr, m_b.get_what_name(paddr));
    }
    ...
    void do_write_tlb(TLB_set_index set_index, uint64_t slot_index, uint64_t vaddr_page, uint64_t vp_offset,
        ...
        log_write_access(slot_paddr, SHADOW_TLB_SLOT_LOG2_SIZE,
    ...
    void do_reset_state() {
        ...
        log_write_access(UARCH_STATE_START_ADDRESS, UARCH_STATE_LOG2_SIZE, [this]() {
            m_m.reset_uarch();
