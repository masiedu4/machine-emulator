...
static bool shadow_peek(READ_REG_F read_reg, const pma_entry &pma, const machine &m, uint64_t offset, uint64_t length,
    ...
    // Must be inside range and must be aligned to machine word size
    if (!(length < pma.get_length() && offset < pma.get_length() - length)) {
        *data = nullptr;
        return false;
    }
    // Initial, potentially partial register read
    uint64_t offset_aligned = offset & ~(sizeof(uint64_t)-1);
    if (offset > offset_aligned) {
        auto val = read_reg(m, pma.get_start()+offset_aligned);
    ...
    uint64_t paddr_start = pma.get_start() + offset;
    uint64_t length_aligned = length & ~(sizeof(uint64_t)-1);
    uint64_t paddr_end = paddr_start + length_aligned;
    for (uint64_t paddr = paddr_start; paddr < paddr_end; paddr += sizeof(uint64_t)) {
        auto val = read_reg(m, paddr);
