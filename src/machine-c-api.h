...
/// \brief Send CMIO response from a data buffer
/// \param m Pointer to valid machine instance
...
/// must be deleted by the function caller using cm_delete_cstring
/// \returns 0 for success, non zero code for error
CM_API int cm_send_cmio_response(cm_machine *m, uint16_t reason, const unsigned char *data, size_t length, char **err_msg);
