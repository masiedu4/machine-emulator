...
public:
    ...
    replay_step_state_access(unsigned char *log_image, uint64_t log_size, const hash_type &root_hash_before) {
        ...
        // set sibling hashes
        if (!validate_and_advance_offset(log_size, first_sibling_offset, sizeof(hash_type), m_sibling_count,
                &end_offset)) {
            interop_throw_runtime_error("sibling hashes past end of step log");
        }
        // NOLINTNEXTLINE(cppcoreguidelines-pro-type-reinterpret-cast)
        m_sibling_hashes = reinterpret_cast<hash_type *>(log_image + first_sibling_offset);
...
private:
    ...
    page_type *try_find_page(host_addr haddr_page) const {
        ...
        while (min < max) {
            auto mid = (min + max) >> 1;
            auto mid_page_data = cast_ptr_to_host_addr(m_pages[mid].data);
